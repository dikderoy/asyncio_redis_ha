...
class RedisProtocolTest(TestCase):
    ...
    def test_bitops(self, transport, protocol):
        ...
        bytes_transport, bytes_protocol = yield from connect(self.loop,
...
class RedisBytesProtocolTest(TestCase):
    ...
    def test_pubsub(self, transport, protocol):
        ...
        def listener():
            # Subscribe
            transport2, protocol2 = yield from connect(self.loop,
                                                       protocol=lambda **kw: RedisProtocol(encoder=BytesEncoder(),
                                                                                           **kw))
...
class RedisPoolTest(TestCase):
    ...
    def create_pool(self, **kwargs):
        return self.pool_class.create(host=HOST, port=PORT, **kwargs)
    ...
    def test_parallel_requests(self):
        ...
        def test():
            # Create connection
            connection = yield from self.create_pool(poolsize=2)
    ...
    def test_select_db(self):
        ...
        @asyncio.coroutine
        def test():
            c1 = yield from self.create_pool(poolsize=10, db=1)
            c2 = yield from self.create_pool(poolsize=10, db=2)

            c3 = yield from self.create_pool(poolsize=10, db=1)
            c4 = yield from self.create_pool(poolsize=10, db=2)
    ...
    def test_in_use_flag(self):
        ...
        def test():
            # Create connection
            connection = yield from self.create_pool(poolsize=10)
            for i in range(0, 10):
    ...
    def test_lua_script_in_pool(self):
        ...
        def test():
            # Create connection
            connection = yield from self.create_pool(poolsize=3)
    ...
    def test_connection_reconnect(self):
        ...
        @asyncio.coroutine
        def test():
            connection = yield from self.create_pool(poolsize=1)
            yield from connection.set('key', 'value')
    ...
    def test_connection_lost(self):
        ...
        def test():
            # Create connection
            connection = yield from self.create_pool(poolsize=1, auto_reconnect=False)
            yield from connection.set('key', 'value')
...
class ExtendedRedisProtocolTest(RedisProtocolTest):
    def setUp(self):
        self.loop = asyncio.get_event_loop()
        self.protocol_class = ExtendedProtocol
...
class SentinelProtocolTest(TestCase):
    ...
    @redis_test
    def test_slaves(self, transport, protocol):
...
class GuardedPoolTest(RedisPoolTest):
    ...
    def setUp(self):
        self.loop = asyncio.get_event_loop()
        self.pool_class = ConnectionManager
    ...
    def test_connection_lost(self):
        ...
        def test():
            ...
            # Close sentinels to make discover and recovery fail
            """:type ConnectionManager"""
            for s in connection._sentinels:
                s.close()
            # Ruin configuration to prevent reopenning of sentinel connections
